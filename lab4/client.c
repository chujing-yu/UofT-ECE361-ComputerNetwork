...
void *receive(void *socketfd_void_p) {
  ...
  for (;;) {	
    ...
    if (packet.type == JN_ACK) {
	  ...
	  //printf("packet.type == NS_ACK\n");
	  fprintf(stdout,"%s\n",packet.data);
	  insession=true;
    } else if(packet.type==MESSAGE){
	  fprintf(stdout, "from: %s,receive message: %s\n",packet.source, packet.data);
	} else if(packet.type==QU_ACK){
      fprintf(stdout, "session_name  member:\n \t%s\n", packet.data);
	}else{
...
void login(char *pch, int *socketfd_p, pthread_t *receive_thread_p) {
	...
	pch = strtok(NULL, " \n");
	server_port = pch;
	strncpy(client_ID, client_id, MAX_NAME);
	//client_ID=client_id;
...
void createsession(char *pch,int socketfd) {
  ...
  pch = strtok(NULL, " ");
  new_sessionID = pch;
  strncpy(packet.data,new_sessionID, MAX_DATA);
  packet.size = strlen(packet.data);
  packetToString(&packet, buf);		
  if ((numbytes = send(socketfd, buf, BUF_SIZE - 1, 0)) == -1) {
    fprintf(stderr, "client: send\n");
